...
## Deviations from original tutorial

I ran into an authentication bug caused by me using a newer version of PowerShell (7.5.1) and Microsoft Graph PowerShell SDK (2.28.0).
[GitHub Issue 3139](https://github.com/microsoftgraph/msgraph-sdk-powershell/issues/3197#issuecomment-2896474896)

```powershell
# The workaround is the addition of: -ContextScope Process
Connect-MgGraph -ClientId $clientId -TenantId $authTenant -Scopes $graphScopes -UseDeviceAuthentication -ContextScope Process -NoWelcome
```

I did not use app-only authentication.
